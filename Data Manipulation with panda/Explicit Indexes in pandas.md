...
set_index()
```
does not change the original DataFrame unless you save the result or use `inplace=True`.
Recommended style:
...
```
中文理解：  
`set_index()` 会返回一个新的 DataFrame。通常要赋值给一个新变量。

---

## 4. Resetting an Index

To move the index back into the DataFrame as a normal column, use:
...
Sometimes you do not want to keep the index values.
...
中文理解：  
`drop=True` 表示不要把原来的 index 放回表格，直接丢掉。
...
- `breed` is the outer level.
- `color` is the inner level.
